...
from networks import (DCGANDiscriminator, Generator, WGANDiscriminator,
                      weights_init)
...
def init_training(args):
    ...
    discriminator = {"dcgan": DCGANDiscriminator, "wgan_gp": WGANDiscriminator}[
        args.gan_type
    ]()
...
def training_step_wgan_gp(
    batch_idx, batch, device, train_config, generator, discriminator, optimizers
):
    ...
    grad = grad[0]
    grad_penalty = (
        train_config["grad_penalty_weight"]
    ...
    for epoch in range(train_config["start_epoch"], train_config["max_epoch"] + 1):
        for batch_idx, batch in enumerate(data_loader):
            if args.gan_type == "dcgan":
                ...
                _, d_loss = training_step_wgan_gp(
                    batch_idx,
                    batch,
                    device,
                    train_config,
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        help="Dataset to use.",
        required=True,
        choices=["cats", "flowers"],
    )
    parser.add_argument(
        "-p",
        "--data_path",
        type=str,
        help="Download path for the data.",
        default="./data",
    )
    parser.add_argument(
        "-c",
