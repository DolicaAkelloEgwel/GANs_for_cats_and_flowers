...
def rot_image(img, angle, center):
    ...
    rotated_img = cv2.warpAffine(
        img, rot_mat, (width, height), borderValue=(128, 128, 128)
    )
    return rotated_img
...
def rotate_image_and_keypoints(img, keypoints, angle):
    ...
    for point_name, (px, py) in keypoints.items():
        qx = (
            center[0]
            + math.cos(-angle) * (px - center[0])
            - math.sin(-angle) * (py - center[1])
        )
        qy = (
            center[1]
            + math.sin(-angle) * (px - center[0])
            + math.cos(-angle) * (py - center[1])
        )
...
def crop_catface(img_path, annotation_path):
    ...
    if keypoints["right_ear_base"][0] < keypoints["left_ear_base"][0]:
        keypoints["left_ear_base"], keypoints["right_ear_base"] = (
            keypoints["right_ear_base"],
            keypoints["left_ear_base"],
        )
    ...
    center = (
        keypoints["right_eye"][0] * 0.5 + keypoints["left_eye"][0] * 0.5,
...
class Catfaces64Dataset(Dataset):
    ...
    def create_from_scratch(cls, data_path):
        ...
        catface_img_paths = cls.extract_catfaces(
            img_paths,
            annotation_paths,
            os.path.join(data_path, "cat_data", "cropped_catfaces"),
        )
        # define dataset
    ...
    def prepare_cat_data(data_path="./data"):
        ...
        # download
        if not os.path.exists(dataset_zip1_path) or not os.path.exists(
            dataset_zip2_path
        ):
            print("Downloading cat dataset part 1...")
            os.system(
                "wget https://web.archive.org/web/20150520175555/"
                "http://137.189.35.203/WebUI/CatDatabase/Data/CAT_DATASET_01.zip -P {}".format(
                    cat_data_path
                )
            )
            ...
            os.system(
                ...
                "http://137.189.35.203/WebUI/CatDatabase/Data/CAT_DATASET_02.zip -P {}".format(
                    cat_data_path
                )
            )

        # unzip
        if not os.path.exists(dataset_part1_path) or not os.path.exists(
            dataset_part2_path
        ):
            print("Unziping dataset part 1...")
            with zipfile.ZipFile(
                "{}/CAT_DATASET_01.zip".format(cat_data_path), "r"
            ) as z:
                z.extractall("{}/CAT_DATASET_01".format(cat_data_path))
            print("Unziping dataset part 2...")
            with zipfile.ZipFile(
                "{}/CAT_DATASET_02.zip".format(cat_data_path), "r"
            ) as z:
            ...
            os.system(
                ...
                "http://137.189.35.203/WebUI/CatDatabase/Data/00000003_015.jpg.cat -P {}".format(
                    cat_data_path
                )
            )
            os.system(
                "mv {}/00000003_015.jpg.cat {}/CAT_DATASET_01/CAT_00".format(
                    cat_data_path, cat_data_path
                )
            )
            os.system(
                "rm {}/CAT_DATASET_01/CAT_00/00000003_019.jpg.cat".format(cat_data_path)
            )

        print("Dataset prepared.")
        img_paths = [
            os.path.join(dp, f)
            for dp, dn, fn in os.walk(os.path.expanduser(cat_data_path))
            for f in fn
            if f.endswith(".jpg") and "/cropped_catfaces/" not in dp
        ]
        annotation_paths = [
            os.path.join(dp, f)
            for dp, dn, fn in os.walk(os.path.expanduser(cat_data_path))
            for f in fn
            if f.endswith(".jpg.cat")
        ]
        img_paths.sort()
    ...
    def extract_catfaces(img_paths, annotation_paths, catfaces_path):
        ...
        if not os.path.exists(catfaces_path):
            ...
            for idx in range(len(img_paths)):
                ...
                cv2.imwrite(
                    "{}/{}_{}".format(
                        catfaces_path, img_path.split("/")[-2], img_path.split("/")[-1]
                    ),
                    img,
                )
        else:
        ...
        catface_img_paths = [
            os.path.join(catfaces_path, f)
            for f in os.listdir(catfaces_path)
            if f.endswith(".jpg")
        ]
